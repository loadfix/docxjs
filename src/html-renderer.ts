...
// Five lowercase hex chars (20 bits, ~1M possible values). Generated by the
// platform CSPRNG so two simultaneous `renderAsync` calls that happen to
// share a `className` emit disjoint CSS counter names — see issue #181.
// The value only ever appears in class names and CSS counter identifiers
// that the renderer produces itself; it is never interpolated with DOCX
// data. Falls back to Math.random() only if a CSPRNG is unavailable (very
// old Node / embedded shells) — the uniqueness guarantee in that case is
// weakened but the API surface still works.
function generateRenderSessionId(): string {
	try {
...
export class HtmlRenderer {
	...
	// Per-render short random suffix appended to every numbering class and
	// CSS counter name emitted by this renderer. Two renders on the same
	// page with the same `className` (the default `"docx"`) used to share
	// CSS counter state because `p.docx-num-1-0 { counter-increment: docx-num-1-0 }`
	// rules from render A would match paragraphs in render B — continuing
	// numbering from preview 1 into preview 2 (issue #181). The suffix is
	// generated in `render()` (once per invocation) using crypto-random
	// bytes so it never collides between renders. See `numberingClass` /
	// `numberingCounter`. Only non-empty when a render is in progress.
	private renderSessionId: string = '';
	...
	async render(document: WordDocument, options: Options): Promise<Node[]> {
		...
		this.h = options.h ?? h;
		// Crypto-random per-render suffix — fixes #181 (numbering continues
		// across multiple previews). 5 hex chars = 20 bits = ~1e-6 collision
		// odds for a handful of renders, well within range for a CSS id.
...
`;
	...
	numberingClass(id: string, lvl: number) {
		// Suffix the class with the per-render session id so two renderers
		// mounted on the same page cannot cross-match each other's
		// numbering selectors. The class is always paired 1-1 with a
		// counter emitted by `numberingCounter` below, so keeping them in
		// lock-step (same suffix) avoids a double-rewrite of the CSS.
		// Empty suffix is only seen outside a render() — never on a live
		// page — so we skip the trailing dash for readability.
	...
	numberingCounter(id: string, lvl: number) {
		// Counter names carry the per-render session suffix so two
		// renderers with the same `className` don't share counter state
		// (issue #181). The suffix is hex-only — safe as a CSS ident.
		return this.renderSessionId
			? `${this.className}-num-${id}-${lvl}-${this.renderSessionId}`
			: `${this.className}-num-${id}-${lvl}`;
	}
